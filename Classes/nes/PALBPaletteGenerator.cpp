...
PALBPaletteGenerator::PALBPaletteGenerator()
{
...
int PALBPaletteGenerator::wave(int p, int color)
{
    return ((color + p + 8) % 12 < 6) ? 1 : 0;
}

float PALBPaletteGenerator::gammafix(float f, float gamma)
{
    return (float)(f < 0.0f ? 0.0f : powf(f, 2.2f / gamma));
}

int PALBPaletteGenerator::clamp(float v)
{
    return (int)(v < 0 ? 0 : v > 255 ? 255 : v);
}

int PALBPaletteGenerator::MakeRGBcolor(int pixel)
{
    ...
    for (int p = 0; p < 12; p++) // 12 clock cycles per pixel.
    {
        ...
        i += (float)(v * cos((M_PI / 6.0) * (p + 0.5F + hue_tweak)));
        q += (float)(v * sin((M_PI / 6.0) * (p + 0.5F + hue_tweak)));
    }
...
int* PALBPaletteGenerator::GeneratePalette()
{
    for (int i = 0; i < 512; i++)
...
int* PALBPaletteGenerator::GeneratePaletteGBR()
{
    for (int i = 0; i < 512; i++)
    {
